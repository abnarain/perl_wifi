...
Format:  interface, rx_packets,rx_bytes,tx_packets , tx_bytes, tx_retries,tx_failed ,tx_rate_i.tx_rate_d, rx_rate_i.rx_rate_d ,-signal_avg, timestamp
...
$index=0;
if (@ARGV!=2){
     print "Usage: ./client-general-data-calc.pl <input data directory> <output data directory> \n" ;
     exit(1);
}
...
#for finding the change in retransmission and dump it into a file
	...
	$diff_line[1]=$current[1]-$previous[1];	$diff_line[2]=$current[2]-$previous[2];
		...
		$diff_line[11]=$current[10]-$previous[10];
		if ($diff_line[1]<0){
		    $diff_line[1] = $current[1];
		}
		if ($diff_line[2]<0){
		    $diff_line[2] = $current[2];
		}
		if ($diff_line[3]<0){
		    $diff_line[3] = $current[3];
		}
		if ($diff_line[4]<0){
                    $diff_line[4] = $current[4];
		}
		$diff_line[12]= $diff_line[1]*$diff_line[9]; # total signal strength= no. of rx packets * signal strength
                $diff_line[13]= $diff_line[2]*$diff_line[8]; # rx n/w utilization= no. of rx bytes * rx bitrate        
                $diff_line[14]= $diff_line[4]*$diff_line[7]; # tx n/w utilization = no. of tx bytes * tx bitrate
		...
		$diff_line[9]=$current[9];$diff_line[10]=$current[10];
		$diff_line[11]=$current[10]-$previous[10];
                if($diff_line[1]<0){		 #packet count must be positive
                    $diff_line[1]=$current[1];
      		    $error_count++;
                }
                if($diff_line[2]<0){		 #packet count must be positive
                    $diff_line[2]=$current[2];
      		    $error_count++;
                }
		if ($diff_line[3]<0){
		    $diff_line[3] = $current[3];
                ...
                if($diff_line[4]<0){		 #packet count must be positive
                    $diff_line[4]=$current[4];
      		    $error_count++;
                }
		$diff_line[12]= $diff_line[1]*$diff_line[9]; # total signal strength= no. of rx packets * signal strength
	...
	{
	  #The interface has changed hence the 'previous' array won't work  print "change in interface $current[0], $previous[0]\n";
	    @previous=@current;
	    $i=$i+1;
	    @current= split(/\s+/, $lines[$i]);
#	    print "after change in interface $current[0], $previous[0]\n";
    ...
    print "error count for ", $f, " = ", $error_count, "out of total lines ", $i, "\n";  
    $error_count=0;
}
`mkdir -p $ARGV[1]` ;
=s
The following format of output files 
 interface, rx_packets,rx_bytes,tx_packets , tx_bytes, tx_retries,tx_failed ,tx_rate, rx_rate ,-signal_avg, timestamp, signal_sum,
 rx bytes*bitrate, tx byte*bitrate
...
foreach $key(sort keys %mac_add){
    ...
    for( $r_s_i=0;$r_s_i<scalar(@s);$r_s_i++){        
	  ...
	  ${$s[$r_s_i]}[8], " ",${$s[$r_s_i]}[9],  " ", ${$s[$r_s_i]}[10], "  ", ${$s[$r_s_i]}[11],  "  ", 
          ${$s[$r_s_i]}[12] ,  "  ", ${$s[$r_s_i]}[13]  ,  "  ", ${$s[$r_s_i]}[14],"\n";    
