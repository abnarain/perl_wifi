...
if (@ARGV!=2){
    print "Usage: ./client-airtime-plot.pl <input data directory>  <file name>.pdf\n" ;
    exit(1);
}

$filename= "'airtime-time-".$ARGV[1]. ".pdf'";
$a ="
...
set xlabel \" time \"
set ylabel \" network usage (txbytes *avg bitrate(per minute)) \" \n";
...
foreach $f(@files){
    ...
    print FILE "'./",$f,"' using 11:13 w p ps -1 pt ", $color++," title '", $f ,"' , ";
