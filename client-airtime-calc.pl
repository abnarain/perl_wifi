...
$index=0;
if (@ARGV!=2){
    print "Usage: ./client-airtime-calc.pl <input data directory> <output data directory> \n" ;
    exit(1);
}
...
#for finding the change in retransmission and dump it into a file
	...
	#	print " in usual time lapse \n";
		...
		$diff_line[12]= $diff_line[4]*$diff_line[7]; #airtime utilization
		...
		$diff_line[9]=$current[9];$diff_line[10]=$current[10];
		$diff_line[11]=$current[10]-$previous[10];
		$diff_line[12]= $diff_line[4]*$diff_line[7]; #airtime calculation
		if ( $diff_line[4]>0 ){
		    push(@{$mac_add{$f}},\@diff_line);
		}else{
#		    print "error line is",join(" ", @diff_line),"\n";
#		    $error_count++;
		    $error_count++;
		}
	    }
...
foreach $key(sort keys %mac_add){
	...
	${$s[$r_s_i]}[8], " ",${$s[$r_s_i]}[9],  " ", ${$s[$r_s_i]}[10], "  ", ${$s[$r_s_i]}[11],  "  ", ${$s[$r_s_i]}[12], "\n";    
    }
    close(FILE);
}
