...
#next three lines are for format
set xdata time 
set format
set timefmt \"%s\"
...
closedir(DIR);
print $ARGV[0];
open (FILE, ">>./$ARGV[0]/plot-signal-timeline.gnu") or die "Cannot create gnu plot file";
print FILE $a;
$count =0;
$color=1;
foreach $f(@files){
    ...
    print $f , "   " ;
    print FILE "\'./",$f,"\' using 11:10  w p ps -1 pt ", $color++," title '", $f ,"' ,";
